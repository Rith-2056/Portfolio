...
You drive a hover rover around a floating island. Each location on the island is a section of the portfolio. Approach one, press `E`, and a clean 2D panel materialises with the content.
...
components/
  3d/                the world: island, rover, camera rig, zone wrapper, ambient effects
...
- No textures or model files. Every object is procedural low-poly geometry with a shared set of materials, which keeps the download to code only and keeps draw calls low.
